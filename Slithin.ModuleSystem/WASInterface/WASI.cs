...
namespace Slithin.ModuleSystem.WASInterface
{
    public class WASI
    {
        public WASI(Module module)
        {
            ...
            Sg_wasm.__mem = Memory.Start;
            Sg_wasm.__mem_size = (int)Memory.Size;
        }
        ...
        {
            ImportDictionary importss = new ImportDictionary();
            importss.Add("wasi_unstable", new Dictionary<string, RuntimeImport>
            {
                ...
                ["path_open"] = new FunctionImport(new Func<int, int, int, int, int, long, long, int, int, int>(WasiUnstable.path_open)),
